...
/* USER CODE END Boot_Mode_Sequence_1 */
  ...
  while (1)
  {
	 ...
	 while (1)
	 {
	     // Move the servo from -180° (left) to 40° (right)
	     for (float angle = -180.0; angle <= 40.0; angle += 1.0) {
	         Turning_SetAngle(angle);
	         HAL_Delay(20);  // 20ms delay between steps
	     }

	     // Move the servo back from 40° (right) to -180° (left)
	     for (float angle = 40.0; angle >= -180.0; angle -= 1.0) {
	         Turning_SetAngle(angle);
	         HAL_Delay(20);  // 20ms delay between steps
	     }
	 }
